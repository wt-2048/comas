...
from marti.helpers.ring_attention import get_ring_attn_group, set_ring_attn_group
from marti.helpers.distributed.distributed_sampler import DistributedSampler
from marti.helpers.distributed.deepspeed_utils import (
    _z3_params_to_fetch,
    get_eval_ds_config,
...
class DeepspeedStrategy(ABC):
    ...
    def save_model(self, model: nn.Module, tokenizer, output_dir, **kwargs) -> None:
        ...
        if self.is_rank_0():
            ...
            # === 宽容处理：lm_head 相关的 missing keys ===
            state_dict_keys = set(state_dict.keys())
            output_state_dict_keys = set(output_state_dict.keys())

            # 统一处理“权重共享”的场景（Qwen 等经常 tie word embeddings）
            if getattr(model_to_save.config, "tie_word_embeddings", False):
                # 移除各种前缀下的 lm_head.weight
                state_dict_keys = {k for k in state_dict_keys if not k.endswith("lm_head.weight")}

            # 若仍存在缺失的 lm_head 变体键，则把这些缺失键从检查中排除
            def _is_lm_head_key(name: str) -> bool:
                # 兼容多种封装前缀：base_model.model.lm_head.weight / model.lm_head.weight / lm_head.weight ...
                return name.endswith("lm_head.weight")

            missing = state_dict_keys - output_state_dict_keys
            # 允许缺失的 lm_head 变体键直接跳过
            allowed_missing = {k for k in missing if _is_lm_head_key(k)}
            remaining_missing = missing - allowed_missing

            if allowed_missing:
                self.print(f"[deepspeed.save_model] skip tied lm_head keys (not gathered): {sorted(allowed_missing)}")

            if remaining_missing:
                # 对于非 lm_head 的缺失键仅做告警，不中断
                self.print(f"[deepspeed.save_model] WARNING: missing gathered keys: {sorted(remaining_missing)}")
                # 可选：把这些键也从比较集合里剔除，避免阻塞保存
                state_dict_keys = state_dict_keys - remaining_missing

            # 过去这里是 assert；我们改为宽容处理，继续保存
