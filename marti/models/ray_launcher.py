...
class PPORayActorGroup:
    ...
    def async_fit_actor_model(self, steps, shared_data_refs, pretrain_dataset=None):
        """Train actor model.
        Returns:
            List: list of remote object refs.
        """
        refs = []
        for i, actor in enumerate(self._actor_handlers):
            refs.append(actor.fit.remote(steps, shared_data_refs[i], pretrain_dataset))

        return refs

    def async_init_credit_trainer(
            self,initial_model_group: "PPORayActorGroup",
        ):
        """
        Init trainer for each credit worker
        """
        initial_actors = initial_model_group._actor_handlers
    ...
    def async_fit_credit_model(self, steps, credit_samples):
        """Train actor model.
        Returns:
            List: list of remote object refs.
        """
        refs = []
        for i, actor in enumerate(self._actor_handlers):
            refs.append(actor.fit.remote(steps, credit_samples))

        return refs

    def async_fit_and_reward_credit_model(self, steps, credit_samples):
        """Train actor model.
        Returns:
            List: list of remote object refs.
        """
        refs = []
        for i, actor in enumerate(self._actor_handlers):
            refs.append(actor.fit_and_reward.remote(steps, credit_samples))
