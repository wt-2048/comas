...
class CriticModelRayActor(BasePPORole):
    def init_model_from_pretrained(self, 
        ...
        # load checkpoint
        if args.load_checkpoint and os.path.exists(os.path.join(args.ckpt_path, "_" + self.rolename)):
    ...
    def reload_states(self):
        reload_deepspeed_states(self.critic.model)

    def offload_states(self):
        offload_deepspeed_states(self.critic.model)
    ...
    def save_model(self, save_path=None):
        ...
        self.strategy.save_model(
            self.critic,
            None,
            save_path if save_path is not None else args.save_path + "_" + self.rolename,
        )

    def save_checkpoint(self, tag):
        args = self.strategy.args
        self.strategy.save_ckpt(
            self.critic, os.path.join(args.ckpt_path, "_" + self.rolename), tag, args.max_ckpt_num, args.max_ckpt_mem
        )
