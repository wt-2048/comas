...
def preprocess_data(data, input_template=None, input_key="input", output_key=None, apply_chat_template=None):
    if apply_chat_template:
        if output_key:
            if isinstance(data[input_key], str):
                input_chat = [{"role": "user", "content": data[input_key]}]
                output_chat = [{"role": "assistant", "content": data[output_key]}]
            prompt = apply_chat_template(input_chat, tokenize=False, add_generation_prompt=True)
            response = apply_chat_template(input_chat + output_chat, tokenize=False)[len(prompt) :]
        else:
